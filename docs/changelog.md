...
| 日期 | 类型 | 摘要 |
|------|------|------|
| **2026-05-17** | 架构 / 功能 / 测试 / 文档 | **行程 preset（backend）**：**`src/prompts/presets/schedule.md`** + **`agent_presets.py`**；**`chat_context`/`messages`** 支持 **`extra_system`**；**`ChatRequest.preset`**。**`route_auto`**：删除写死 MCP 工具名与未完成 **`_try_export_mcp_route`**；行程关键词 → **`chat`+`preset=schedule`**；MCP 仍由 **tools/list + LLM** 选 **`mcp_tool`/`mcp_arguments`**。**`chat_stream`**：**`auto`** 写入会话 **`extra_json.preset`**。不在 backend 绑定导出/联网专用逻辑。**测试**：**`test_agent_presets`**、**`test_route_auto`**、**`test_chat_stream`** preset/auto、**`test_chat_context`** extra_system；修复 **`test_conversation_refine`** / **`test_llm_completion`**（**`complete_chat`**）。**文档**：**`docs/agent-presets.md`**、**`readme.md` §7**（本阶段收尾 → **mcp-server**）、**`chat-stream-api.md`** **`preset`** 与 §5。 |
| **2026-05-16** | 功能 / 测试 / 文档 | **SSE `tool_call` / `tool_result`**：**`sse_events`** 新增构造器；**`chat_stream` mcp** 分支调工具前后发出事件，失败时 **`tool_result.is_error`** + **`error`**。**测试**：**`test_chat_stream`** 断言事件顺序、失败路径；**`iter_chat_chunks`** 替换旧 **`iter_ollama_chat_chunks`** mock。**文档**：**`readme.md` §3/§5/§7**、**`docs/chat-stream-api.md` §3/§6/§7**。下一优先：**前端**解析工具事件、**`MCP_ALLOWED_TOOLS`**。 |
| **2026-05-16** | 功能 / 文档 | **MCP 返回值润色**：**`llm/config.mcp_reply_via_llm`**（**`.env`** **`MCP_REPLY_VIA_LLM`**）；**`chat_stream`** mcp 分支在 **`format_call_tool_result`** 后可 **`iter_chat_chunks`** 流式润色，落库 **`meta.mcp_raw`**。**`routing=auto` 增强**：**`route_auto`** 规则兜底（点名调用工具、无 **`inputSchema.required`** 时 **`mcp`+`{}`**）、**`_tool_schema_hint`**、**`extract_first_json_object`**。**文档**：**`readme.md` §3/§4/§7**、**`docs/chat-stream-api.md` §5–§6**、**`docs/llm-providers.md`**。 |
| **2026-05-16** | 架构 / 文档 | **LLM 多提供商**：**`src/providers/`**（**`ollama.py`**、**`zhipu.py`** + **`ChatProvider`** 协议）、**`complete_chat` / `iter_chat_chunks`** 分发；**`src/env.py`** 统一 **`load_dotenv`**（**`api.py`** 首行 import）；移除 **`llm/config`/`db`/`mcp_config`/`providers` 内分散 load**。业务：**`chat_stream`** → **`iter_chat_chunks`**，**`route_auto`/`conversation_refine`** → **`complete_chat`**。新增 **`docs/llm-providers.md`**；更新 **`readme.md` §2/§3/§4/§7**、**`documentation-index.md`**。 |
| **2026-05-16** | 功能 / 文档 | **`routing=auto`**：新增 **`src/services/route_auto.py`**（**`mcp_list_tools_async`** → **`complete_ollama_chat`** 判别 **`chat`/`mcp`** + **`mcp_tool`/`mcp_arguments`**，JSON 围栏截取，未知工具回退 **`chat`**）；**`chat_stream.py`** 接入 **`auto_decision`**，**`mcp`** 在 auto 下用判别结果，**`meta`** 含 **`effective_route`**。**文档**：**`readme.md` §2/§3/§7**、**`docs/chat-stream-api.md` §2/§5/§6/§7/§8**、**`documentation-index.md`**、**`user-profile.mdc`**。下一优先：**MCP 结果 LLM 润色**、SSE **`tool_call`/`tool_result`**、白名单；**WebSocket 暂非必须**。 |
| **2026-05-16** | 功能 / 测试 / 文档 | **`routing=mcp`**：`ChatRequest` 增加 **`mcp_tool`/`mcp_arguments`**；**`chat_stream`** 调 **`mcp_call_tool_async`** 并 SSE **`delta`+落库**；**`format_call_tool_result`**。**`chat_context`**：上下文拼装单一入口。**测试**：**`test_chat_stream`** MCP 三用例、**`test_mcp_client`**、**`test_chat_context`**（**`pytest` 24 passed**）。**`readme.md` §3/§7**、**`docs/chat-stream-api.md`** §2/§5/§6/§7 同步；下一优先 **`auto`/plan 自动选工具** 与前端。 |
